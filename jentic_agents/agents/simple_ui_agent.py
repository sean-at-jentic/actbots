...
class SimpleUIAgent(BaseAgent):
    ...
    def _create_chat_area(self) -> None:
        ...
        # Welcome message
        self._add_bot_message("👋 Welcome to ActBots! I'm your AI assistant. What can I help you with today?")
    ...
    def _create_input_area(self) -> None:
        ...
        input_wrapper = tk.Frame(input_frame, bg='#f3f4f6', relief='flat', borderwidth=1)
        ...
        self.input_text.bind('<Command-Return>', lambda e: self._send_message())  # For Mac
    
    def _on_input_focus_in(self, event):
        """Handle input focus in event."""
        if self.input_text.get(1.0, tk.END).strip() == "Type your message here... (Ctrl+Enter to send)":
    ...
    def _handle_goal_error(self, goal: str, error_msg: str) -> None:
        ...
        self._add_bot_message(f"I encountered an error while processing your request:\n\n{error_msg}", "error")
...
The AI assistant can help with research, planning, coding questions, and much more!"""
    ...
    def handle_output(self, result: ReasoningResult) -> None:
        ...
        if result.success:
            ...
            # Error output
            error_response = f"I wasn't able to complete that request.\n\n{result.final_answer}"
