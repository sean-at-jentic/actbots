...
class MockHITLAgent:
    ...
    def __init__(
        self, inbox: CLIInbox, outbox: CLIOutbox, intervention_hub: CLIInterventionHub
    ):
        self.inbox = inbox
        self.outbox = outbox
            ...
            for goal in self.inbox.goal_stream():
                ...
                try:
                    self.outbox.send_progress(
                        self.current_goal_id, f"Starting work on: {goal}"
                    )
    ...
    def process_goal_with_hitl(self, goal: str) -> str:
        ...
        if self.is_goal_ambiguous(goal):
            self.outbox.send_progress(
                self.current_goal_id, "Goal appears ambiguous, requesting clarification"
            )
            ...
            goal = clarified_goal
            self.outbox.send_progress(
                self.current_goal_id, f"Goal clarified to: {goal}"
            )
        ...
        if len(plan) > 3:
            self.outbox.send_progress(
                self.current_goal_id, "Plan is complex, requesting human review"
            )
        ...
        for i, step in enumerate(plan, 1):
            self.outbox.send_progress(
                self.current_goal_id, f"Executing step {i}/{len(plan)}: {step}"
            )
            ...
            try:
                ...
                self.outbox.send_step_complete(self.current_goal_id, step, step_result)
                ...
                self.outbox.send_progress(
                    self.current_goal_id, f"Applying human guidance: {guidance}"
                )
        ...
        if len(results) > 1:
            ...
            if decision == 0:
                final_result = (
                    f"Summary of {len(results)} completed steps: " + "; ".join(results)
                )
            elif decision == 1:
    ...
    def is_goal_ambiguous(self, goal: str) -> bool:
        ...
        return (
            any(word in goal.lower() for word in ambiguous_words)
            or len(goal.split()) < 3
        )
    ...
    def generate_plan(self, goal: str) -> list[str]:
        ...
        if "send" in words or "email" in words:
            ...
            return ["Analyze the request", "Execute the task", "Verify completion"]
